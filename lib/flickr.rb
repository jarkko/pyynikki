...
class Flickr
  ...
  def photos(*criteria)
    ...
    return [] unless photos['photos']['photo']
    photos['photos']['photo'].collect { |photo| Photo.new(photo['id'], photo, self) }
  end
  ...
  class Photo
    ...
    attr_reader :id, :client, :attrs

    def initialize(id=nil, attrs = {}, client = nil)
      @id = id
      @attrs = attrs
    ...
    def url(size='Medium')
      if size=='Medium'
        if @client.host == "http://www.23hq.com"
          "#{@client.host}/#{owner.username}/photo/#{id}"
        else
    ...
    def tt_source(size = 'Medium')
      ...
      "#{@client.host}/#{@attrs['server']}/#{id}_#{@attrs['secret']}_#{size}.jpg"
    end
