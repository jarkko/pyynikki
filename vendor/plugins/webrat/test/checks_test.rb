...
class ChecksTest < Test::Unit::TestCase
  ...
  def test_should_send_checked_check_box_as_one_even_with_hidden_input
    @response.stubs(:body).returns(<<-EOS)
      <form method="post" action="/login">
        <label for="user_tos">I agree to the bloody TOS</label>
        <input id="user_tos" checked="checked" name="user[tos]" value="1" type="checkbox" />
        <input name="user[tos]" type="hidden" value="0" />
        <input type="submit" />
      </form>
    EOS
    @session.expects(:post_via_redirect).with("/login", "user" => {"tos" => "1"})
    @session.clicks_button
  end
