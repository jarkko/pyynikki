...
module ActionController
  module Integration
    ...
    class Session
      ...
      # Example:
      #   selects "January"
      def selects(option_text, opts = {})
        option = find_option_by_value(option_text, opts[:for])
    ...
    protected # Methods you could call, but probably shouldn't
      ...
      def find_option_by_value(option_value, scope = nil) # :nodoc:
        option_nodes(scope).detect { |el| el.innerHTML == option_value }
      end
      ...
      def add_default_params_for(form) # :nodoc:
        ...
        add_default_params_from_select_lists_for(form)
      end
      ...
      def add_default_params_from_select_lists_for(form) # :nodoc:
        (form / "select").each do |select|
          option = (select / "option[@selected]").first || (select / "option").first
          add_form_data(select, option.attributes["value"] || option.innerHTML) if option          
        end
      end
      ...
      def option_nodes(scope = nil) # :nodoc:
        if scope
          (dom / "##{scope}" / "option")
        else
          (dom / "option")
